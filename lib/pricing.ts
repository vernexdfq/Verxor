...
export const MAX_PROVIDER_COST_USD = 2;
...
/** Map provider cost → Server 1/2 + public tier. Reject over max. */
 ...
 * Under 0.5 → S4, 0.5–0.99 → S3, 1–1.49 → S2, 1.5–2 → S1
 */
export function wholesaleSlot(providerCostUsd: number): 1 | 2 | 3 | 4 | null {
  ...
  if (providerCostUsd < 1.5) return 2;
  return 1;
}
