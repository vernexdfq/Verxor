import { ArrowRight, Bell, Clock3, Eye, EyeOff, Globe2, History, KeyRound, LockKeyhole, LogOut, Plus, ShieldCheck, Smartphone, UserRound, WalletCards, Zap } from 'lucide-react';
  ...
  return <>
    <section className="home-heading">
      <div>
        ...
        <h1>Good morning, Denny</h1>
        <p>Manage your numbers, orders and wallet in one place.</p>
      </div>
    ...
    <Card className="wallet-card">
      ...
      <div className="wallet-amount-row"><strong>{money}</strong><button className="balance-visibility" aria-label="Hide balance"><Eye size={18} /></button></div>
    ...
    <div className="quick-grid">
      <button className="quick-card" onClick={() => go('numbers')}><span className="quick-icon"><Smartphone size={21} /></span><strong>Virtual Numbers</strong><small>OTP verification</small><ArrowRight size={16} className="quick-arrow" /></button>
      <button className="quick-card" onClick={() => go('numbers')}><span className="quick-icon"><Clock3 size={21} /></span><strong>Rent a Line</strong><small>Long-term numbers</small><ArrowRight size={16} className="quick-arrow" /></button>
...
export function NumbersPage() {
  return <><section className="page-intro numbers-intro"><p className="eyebrow">NUMBERS</p><h1>Choose a number service</h1><p>Use a virtual number for OTPs or rent a line for longer-term access.</p></section><div className="number-choice-list">
    <button className="number-choice"><span className="number-choice-icon"><Smartphone size={22} /></span><span><strong>Virtual Numbers</strong><small>Get a number for OTP verification</small></span><ArrowRight size={18} /></button>
    <button className="number-choice"><span className="number-choice-icon"><Clock3 size={22} /></span><span><strong>Rent a Line</strong><small>Keep a number for the duration you need</small></span><ArrowRight size={18} /></button>
...
export function ProfilePage({ openService }: { openService: (view: ServiceView) => void }) {
  return <><section className="profile-summary"><div className="profile-avatar">V</div><div><p className="eyebrow">ACCOUNT</p><h1>Your profile</h1><p>Member since 2026</p></div></section>
    <ProfileGroup title="ACCOUNT INFORMATION">
      ...
      <ProfileItem icon={<Bell size={19} />} title="Notifications" description="Order and account alerts" onClick={() => openService('alerts')} />
    </ProfileGroup>
    ...
    <ProfileGroup title="SUPPORT">
      <ProfileItem icon={<Globe2 size={19} />} title="Help Center" description="Guides and common questions" onClick={() => openService('privacy')} />
      <ProfileItem icon={<ShieldCheck size={19} />} title="Contact Support" description="Get help with an issue" onClick={() => openService('feedback')} />
    </ProfileGroup>
    <button className="logout-button"><LogOut size={18} /> Log out</button>
  </>;
}

function ProfileGroup({ title, children }: { title: string; children: React.ReactNode }) { return <section className="profile-group"><h2>{title}</h2><Card className="profile-list">{children}</Card></section>; }
function ProfileItem({ icon, title, description, onClick }: { icon: React.ReactNode; title: string; description: string; onClick?: () => void }) { const Tag = onClick ? 'button' : 'div'; return <Tag className="profile-item" onClick={onClick}><span className="profile-item-icon">{icon}</span><span><strong>{title}</strong><small>{description}</small></span>{onClick && <ArrowRight size={17} />}</Tag>; }
