/**
 * Rental workspace — UI structure ported from vernexdfq/vernex-boost-hub
 * (src/components/rental + src/routes/_authenticated/rental).
 *
 * Adapted for Verxor Vite SPA:
 * - No TanStack Router / React Query / Supabase server functions
 * - Local state + empty/demo data until provider layer is connected
 * - Lives inside Verxor AppShell deep-service mode
 */
...
function EmptyState({ title, text, icon, action }: { title: string; text: string; icon: ReactNode; action?: ReactNode }) {
  return (
  ...
  return (
    <div className="sheet-layer">
      ...
      <div className="sheet">
        ...
        {lines.map((line) => (
          <button className="sheet-row" key={line.id} onClick={() => onSelect(line)}>
...
function Catalog({ country, onBack }: { country: Country; onBack: () => void }) {
  ...
  return (
    <div className="catalog-page">
      ...
      {picked && (
        <div className="catalog-checkout">
          <strong>{picked}</strong>
          <div className="plan-row">
            {plans.map((p) => (
              <button className={plan === p ? 'selected' : ''} key={p} onClick={() => setPlan(p)}>
                {p}
              </button>
            ))}
          </div>
          <strong className="catalog-price">Provider price · {plan}</strong>
          <button className="rent-submit" onClick={() => setPicked(null)}>
            Rent this number
          </button>
        </div>
      )}
  ...
  return (
    <div className="rental-page">
      <header className="rental-header">
        <button className="rental-icon-button" onClick={onBack} aria-label="Back to Verxor">
          <ArrowLeft size={20} />
        </button>
        <h1 className="rental-page-title">{tabTitle}</h1>
        <div className="rental-head-actions">
          {tab === 'calls' || tab === 'numbers' ? (
            <button className="rental-icon-button primary" onClick={openBrowse} aria-label="Rent number">
              <Plus size={22} strokeWidth={2.2} />
            </button>
          ) : tab === 'messages' ? (
            <button className="rental-icon-button primary" aria-label="New message">
        ...
        <>
          {callTab === 'history' && (
            <section className="rental-content">
              ...
              <EmptyState
                ...
                icon={<Clock size={40} />}
                action={
                  <button className="blue-pill" onClick={openBrowse}>
                    Rent a number
                  </button>
                }
              />
          ...
          {callTab === 'keypad' && (
            <section className="keypad-section">
              <button className="from-pill" onClick={() => setShowFrom(true)}>
                {fromLine.label}
                <ChevronDown size={14} />
              </button>
              <div className="dial-display">
                <button onClick={openDialCountry}>
                  {country.flag}
                  <ChevronDown size={14} />
                </button>
              ...
              <div className="dial-grid">
                {KEYS.map(({ digit, letters }) => (
                  <button key={digit} onClick={() => setDial((d) => (d.length >= 18 ? d : d + digit))}>
                    <strong>{digit}</strong>
              ...
              <div className="dial-actions">
                ...
                <button className="call-button" aria-label="Call">
                  <Phone size={27} fill="white" strokeWidth={0} />
                </button>
                <button className="delete-button" onClick={() => setDial((d) => d.slice(0, -1))} aria-label="Delete">
                  <Delete size={17} />
                </button>
              </div>
              <button className="browse-pill" onClick={openBrowse}>
                <Hash size={14} /> Browse & rent numbers
              </button>
            </section>
          )}

          {/* Vernex-style floating sub-tabs for Calls */}
      ...
      {tab === 'credit' && (
        <section className="rental-content credit-section">
          ...
          <ul className="package-list">
            {PACKAGES.map((v) => (
              <li key={v}>
                <button className="package-row" type="button" onClick={onBack}>
                  <div>
                    <strong>{v}</strong>
